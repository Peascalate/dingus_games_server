...
const resync_info = make_struct("id username pos rot")
...
function new_client(packet, ws, clients) {
    ...
    var username = packet.subarray(3,len+3)
    clients[id].uname = username.toString()
    var pkt = construct_packet(0x01, [[id], username])
...
function resync_game(packet,ws,clients) {
    ...
    clients.forEach((v) => {
        ...
        resync_data.push(new resync_info(v.id, v.uname, v.pos, v.rot))
    });
